...
alias b4l="git branch --list 'b4/*'"
alias b4s="b4 shazam -s"
alias b4a="b4 am -s"
alias b4td="b4 ty -d"
alias b4tl="b4 ty -l"
alias b4-presend="rm -rf /tmp/presend && mkdir -p /tmp/presend && b4 send --no-sign -o /tmp/presend; lesspatches /tmp/presend/*"
...
alias glo="git log --format='%C(auto)%h%C(green) %<(9,trunc)%an%C(auto)%d %s'"
alias l="eza --no-git -la"
alias ll="eza --no-git -l"
alias ls="eza --no-git"
alias lsblk="lsblk --tree --output 'NAME,LABEL,MAJ:MIN,RM,SIZE,RO,TYPE,MOUNTPOINTS'"
...
alias wezttys="fd 'tty(USB|ACM)' /dev -x zsh -c 'wezterm start tio {}' 2>&1 >/dev/null & disown"
alias xc="wl-copy -n"
